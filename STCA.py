...
class StackedMultiHeadCrossAttention(Layer):
    ...
    def call(self, query, history):
        outputs = [
            self.layers_list[i](query, history) for i in range(len(self.layers_list))
        ]
        outputs += [self.proj_q(query)]  # target-aware
        outputs = tf.reshape(
            tf.concat(outputs, axis=1), [-1, (self.num_layers + 1) * self.d_model]
        )  # (B,num_layers+1,d_model) -> (B,(num_layers+1)*d_model)
