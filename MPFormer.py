...
class Decoder(Layer):
    def __init__(self, d_model, num_heads, d_ff, **kwargs):
        ...
        self.ffn = tf.keras.Sequential([Dense(d_ff, activation="gelu"), Dense(d_model)])
...
class TaskAwareDecoder(Layer):
    def __init__(self, d_model, num_heads, d_ff, num_experts, top_k, **kwargs):
        ...
        self.ffn_g = tf.keras.Sequential(
            [Dense(d_ff, activation="gelu"), Dense(num_experts), Activation("Softmax")]
        )
        self.ffn_expert = [
            tf.keras.Sequential([Dense(d_ff, activation="gelu"), Dense(d_model)])
            for _ in range(num_experts)
        ]

        self.mlp = Dense(
            d_model * (num_experts + 1), activation="gelu"
        )  # ffn_gate + ffn_expert
...
if __name__ == "__main__":
    ...
    mpformer = MPFormer(
        d_model=32, num_heads=4, d_ff=128, num_experts=4, top_k=2, n_task=1
    )
